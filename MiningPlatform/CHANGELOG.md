...
- Checksum-pinned Bitcoin Core 31.0 regtest image, loopback-only disposable Compose profile, and canonical live trace proving a transaction-bearing `getblocktemplate`, distinct witness txid/wtxid evidence, wallet-owned coinbase construction, network-target candidate solving, proposal acceptance, `submitblock`, exact transaction inclusion, two confirmations, long-poll replacement after a chain-tip change, and exact tip persistence across a node restart without exposing raw block bytes.
