...
- RandomX validation is not yet connected to a miner-facing listener or production sidecar. The dormant producer can now create an immutable contribution only after local and upstream acceptance; rewards, journals, balances, and payouts still require an independently imported, exactly matched upstream settlement. Any failure after durable intent—including a pre-write session/job change whose non-dispatch is known locally—remains unresolved by policy and is never automatically resubmitted until an operator-owned recovery state machine exists.
