...
- Fresh and representative alpha.7/schema-v18-to-schema-v19 migration rehearsals plus controlled-payout, native-evidence, and RandomX submission/outbox integration coverage for historical-data preservation, v15-intent backfill, retry safety, balanced reservation, self-approval rejection, unique final decisions, append-only evidence, concurrent idempotency, proposal freshness, digest correlation, and wallet-reservation oversubscription prevention.
...
- Active root CI and the packaged workflow now run isolated schema-v19 fresh and representative alpha.7/schema-v18 upgrade rehearsals instead of stopping at the released schema-v13 verifier.
...
- RandomX validation is not yet connected to a miner-facing listener or production sidecar. A dormant runtime producer boundary now persists pre-RPC intent and accepted decisions through a transactional outbox, but it is not wired into production services and cannot create a contribution, journal, reward, payout, or balance in this checkpoint. Ambiguous intents require future operator recovery and are never automatically resubmitted.
...
- Exact-tree schema-v19 fresh and representative schema-v18 upgrade rehearsals apply all 19 migrations, preserve historical payout, native-submission, and RandomX evidence fixtures, and verify all RandomX submission/outbox triggers.
- RandomX unit coverage passes 13/13; gateway integration passes 5/5 across accepted/rejected outcomes, durable retry, local rejection, transport ambiguity, concurrent duplicate suppression, transaction rollback, immutable envelopes, and explicit absence of contribution, reward, journal, reservation, or balance effects.
