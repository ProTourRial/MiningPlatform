...
- Added an MFA-protected payout activation-readiness endpoint that reports deterministic environment, database-control, route, wallet, signer, limit, reserve, reconciliation, two-person operator, quarantine, and ambiguous-broadcast blockers without mutating execution state. A companion quarantine queue exposes only non-secret digests and lifecycle evidence, never encrypted transaction artifacts or signer credentials.
...
- Active and packaged browser CI jobs that provision disposable PostgreSQL and Redis services, migrate and seed the real API, keep all payout gates closed, and execute the authenticated desktop/mobile journey without route or network mocks.
- Authenticated browser coverage now simulates an expired access token while retaining its scoped refresh cookie, then proves automatic refresh-token rotation and protected-request replay before continuing the real worker and payout journey.
...
- Schema v19 append-only RandomX upstream-job evidence, locally validated pre-RPC submission intents, immutable upstream decisions, and exact accepted-decision correlation to a transactional outbox event.
- Schema v20 authoritative RandomX dispatch binding with a unique fingerprint over the exact upstream pool, session, job blob, seed, target, height, nonce, and result; existing schema-v19 intents receive a collision-safe immutable backfill without rewriting historical attribution.
- Schema v21 dual-source contribution facts with an exact-one-of invariant for SHA-256 shares or immutable RandomX accepted-share evidence, plus retry-safe backfill events for historical RandomX evidence that retains exact accepted-event correlation.
- Schema v22 payout reorg recovery state with database-gated `COMPLETED → CONFIRMING → COMPLETED` transitions, mandatory append-only chain-regression evidence, consumed-reservation continuity, and an exactly-once reconfirmation counter that forbids a second settlement journal.
- Schema v23 Google OAuth identity foundation with explicit provider-subject binding, one-time authorization attempts, hashed state, nonce, and initiating-browser proof, encrypted PKCE verifier storage, short expiry, and separate sign-in/link purposes. Google provider tokens are never persisted.
- Optional Google Sign-In for explicitly linked active accounts plus password+TOTP step-up link/unlink controls. Matching a verified Google email to an existing password account never auto-links or authenticates it, and the frontend button remains hidden unless the backend reports a complete enabled configuration.
...
- Fresh schema-v23 and representative alpha.7/schema-v13-to-schema-v23 migration rehearsals plus controlled-payout, native-evidence, RandomX submission/outbox/contribution, payout-reorg, and Google OAuth integration coverage for historical-data preservation, staged backfills, retry safety, balanced reservation, append-only evidence, concurrent idempotency, proposal freshness, digest correlation, and replay-resistant identity linking.
...
- Wallet-worker signer client and AES-256-GCM artifact encryption primitives with payout-and-manifest-bound authenticated context, request/response binding, streaming payload limits, timeouts, and regression tests; PSBT, signed PSBT, raw transactions, authorization values, and encrypted artifact fields are redacted from structured logs.
- Regtest-only wallet executor that resumes encrypted PSBT artifacts across process retries, calls the isolated signer, finalizes and preflights the exact transaction, recovers an ambiguous broadcast by observing the deterministic txid before retry, watches confirmation/reorg/drop states, and consumes the reservation only after matched payout, wallet, UTXO, and ledger reconciliation. A dropped transaction—before or after settlement—may replay only its original non-RBF raw transaction; replacement destinations are never synthesized.
- Disposable payout-regtest integration trace and dedicated CI gate covering request eligibility, one-time balance reservation, maker/checker separation, request/approval idempotency, process-isolated signing, Bitcoin Core broadcast, three confirmations, fee-reservation refund, balanced settlement journal, wallet/UTXO/liability reconciliation, forced block invalidation, reconfirmation without a second journal, and replay without a second payout or transaction.
...
- Versioned `mining.randomx.share.accepted.v1` contract and a strict mining-worker consumer that validates the exact bounded payload, required internal producer identifier, account aggregate, acceptance timestamps, and canonical share idempotency key before atomically completing event idempotency and immutable schema-v18 evidence under a PostgreSQL transaction advisory lock.
- Canonical side-effect-free RandomX accepted-share event factory that re-runs accounting projection, requires a bounded blob and uint64 height, normalizes the exact payload/envelope, and refuses rejected or mutated work before a future durable gateway may enqueue it.
- Dormant RandomX submission gateway that persists intent before the upstream side effect, serializes duplicate fingerprints, replays only durable decisions, blocks automatic resubmission after ambiguous transport or persistence outcomes, records rejections without accepted events, and atomically enqueues accepted evidence with its immutable upstream decision.
- Bounded miner-facing CryptoNote JSON-RPC transport for RandomX login, keepalive, assigned-job projection, and submission. It authenticates before issuing work, keeps upstream job identities private, resolves miner assignments before invoking the durable gateway, rate-limits each session, bounds lines/queues/connections/timeouts, revokes identity and work on disconnect, and reports ambiguous post-intent outcomes without encouraging automatic retry. Production runtime activation remains fail-closed until the audited credential adapter and globally unique work allocator are wired.
- Redis-time RandomX work-isolation leases keyed by algorithm, seed, and nonce-normalized hashing blob. Independent gateway replicas may renew only their own lease, cannot issue an identical active 32-bit search space, recheck ownership before resolving a submission, and release with compare-and-delete semantics so one replica cannot delete another owner. This is an admission guard, not a local blob mutation mechanism: the upstream work source must still provide distinct authoritative blobs.
- Dedicated RandomX upstream sessions that authorize one CryptoNote connection per authenticated miner, convert upstream jobs to bounded private miner job IDs, retain only a limited per-connection history, route durable submissions back to the exact owning adapter/coordinator, replace disconnected sessions without reusing stale mappings, and expose new provider-issued work for miner notification. The Redis collision guard remains mandatory because separate TCP sessions do not by themselves prove distinct hashing blobs.
- Production RandomX worker authentication adapter that reuses the SHA-256 listener's PostgreSQL credential, referral-attribution, audit, lockout, and Redis rate-limit policy, requires a mining-account-bound principal, and forwards only an HMAC of the miner agent into the shared authentication boundary. The Stratum application now exposes this narrow worker-authentication subpath without exporting its server runtime.
- Fail-closed RandomX laboratory runtime that composes the production worker-credential boundary, one upstream authorization session per miner, Redis-time cross-replica work leases, sidecar validation, durable pre-RPC intent, upstream dispatch, immutable decision, and transactional accounting outbox behind a loopback-only listener. Startup is disabled by default and code rejects every production-mode activation; the integration trace proves the complete synthetic miner wire path without crediting a balance.
- Active and packaged RandomX CI gates that start disposable PostgreSQL and Redis, apply the complete migration chain, build the exact dependency graph, and exercise validator, upstream protocol, bounded miner transport, cross-replica work isolation, durable submission/outbox, and contribution hand-off without starting a public listener.
- RandomX contribution hand-off that persists accepted evidence and a dedicated outbox event in one transaction; accounting-worker revalidates the event against both the immutable evidence row and original accepted-share outbox before creating exactly one immutable contribution fact for the existing settlement and reward engine.
- Schema-v19 database enforcement for job/account/asset/pool/time correlation, exact accepted decision-to-outbox envelope binding, immutable job/intent/decision evidence, protected accepted-event payloads, and retention exclusion for correlated RandomX outbox evidence.
...
- Checksum-pinned Bitcoin Core 31.0 regtest images, loopback-only disposable Compose profile, and canonical two-node live trace proving a transaction-bearing `getblocktemplate`, distinct witness txid/wtxid evidence, wallet-owned coinbase construction, network-target candidate solving, proposal acceptance, `submitblock`, exact transaction inclusion, two confirmations, long-poll replacement, deterministic longer-chain reorganization to `STALE_CHAIN`, and exact reorg-tip persistence across a node restart without exposing raw block bytes.

### Changed

- Active release identity is synchronized as `0.3.0-alpha.8` across all 35 workspace packages, build defaults, Docker, CI assertions, public UI copy, roadmap, project metadata, and generated manifests; schema identity advances honestly from the previously implemented v22 payout-reorg boundary to the new v23 OAuth migration.
...
- RandomX upstream-decision digests now use the `v2` domain when binding the canonical dispatch fingerprint, preserving a clear verification boundary from historical `v1` decisions without rewriting immutable evidence.
- RandomX adapter lifecycle now shares concurrent starts, defers valid job notifications that arrive in the same TCP chunk as login, ignores callbacks from replaced sockets, clears session/job state on every disconnect, and treats only explicit JSON-RPC errors as terminal rejection; malformed or ambiguous results remain unresolved.
- Contribution accounting now records its source explicitly, preserving existing SHA-256 share attribution while allowing RandomX accepted evidence to enter the same reconciled FOLLOW_UPSTREAM allocation path without synthesizing a Bitcoin `Share` row.
- RandomX share fingerprint v2 now binds the algorithm, full bounded job blob, target, and height in addition to upstream client/job identity and the submitted worker proof, preventing otherwise-identical proof fields from being replayed against mutated work.
- The alpha.7 static release checker now tracks all 33 exact-tree workspace package files and the current independently gated payout and RandomX evidence controls introduced during Unreleased development.
...
- Watch-only Bitcoin reconciliation now binds the confirmed solvable UTXO outpoint set and its canonical digest to the chain-bound wallet snapshot; private-key-disabled descriptor wallets remain usable for PSBT construction while signing stays isolated.
...
- Active root CI and the packaged workflow now run isolated schema-v23 fresh and representative alpha.7/schema-v13 staged upgrade rehearsals instead of stopping at the released schema-v13 verifier.
...
- Manual payout requests in the web control plane retain one intent-bound idempotency key across transport failures and retryable or ambiguous HTTP outcomes, rotate it when the account or amount changes, and clear it after success or a definitive rejection; retrying an uncertain response can no longer create a second reservation under a fresh browser-generated key.
- The Alpha.8 migration verifier now resolves and validates the `DATABASE_URL` schema before any database command and fails closed unless it is `public`, matching the historical migration chain's `pgcrypto` dependency instead of guarding `public` while silently allowing Prisma to target another schema.
- Wallet custody now independently binds a signer-returned PSBT to the approved immutable payout manifest, proves every non-destination output belongs to the watch wallet, and verifies the finalized raw transaction preserves the exact signed input/output intent before any broadcast. Destination, route window, payout wallet, signer-key binding, and global execution controls are rechecked at authoritative database time, with row locks retained across each signer or Bitcoin Core side effect so a concurrent revocation cannot commit between authorization and use. Any post-signer policy change quarantines the payout while retaining its active balance reservation for operator resolution; historical raw-only ambiguous attempts are never rebroadcast automatically and advance only when the exact request digest matches and Bitcoin Core independently observes the decoded transaction ID.
...
- Every chain poll now persists a new append-only observation event even when its state and raw digest match an earlier poll, preserving fresh post-completion evidence required to authorize a confirmation-regression transition.
- Database evidence gates now forbid reservation consumption without a posted payout-settlement journal, forbid broadcast-or-later states without completed signing evidence, and forbid completion without both matched reconciliation and the exact posted settlement linked by the payout.
- Payout reservation, reversal, and settlement journals now insert their lines sequentially inside one transaction, while completion loads asset, route, wallet, and reservation evidence through explicit scalar queries; this avoids concurrent-query warnings on a single PostgreSQL transaction connection without weakening atomicity.
- Clean-checkout migration rehearsals now build every post-migration test workspace graph before executing API, mining, accounting, and RandomX evidence; the payout-regtest workflow explicitly generates its Prisma client before building dependent services and uses Node 24-based checkout/setup actions; and Vercel explicitly builds `@mining/shared` before the web application. None of these gates depend on stale local `dist` artifacts.
...
- The API and web application do not possess private keys and cannot sign or broadcast. Durable wallet orchestration, broadcast recovery, confirmation/reorg observation, and final reconciliation are executable only when every independent gate, a literal `regtest` network selector, and the disposable-regtest acknowledgement are present; the executor rejects mainnet in code and all real-funds gates remain disabled by default.
...
- The RandomX miner-facing transport is runtime-composed only for an explicitly acknowledged, loopback-only laboratory. A Redis-backed cross-replica collision guard deliberately rejects duplicate nonce-normalized blobs instead of inventing an upstream-incompatible nonce range or modifying work locally. Public/production activation, an image-pinned production sidecar, empirical provider distinct-work evidence, HA/partition/load proof, and operator recovery remain gated. Rewards, journals, balances, and payouts still require an independently imported, exactly matched upstream settlement; any failure after durable intent remains unresolved and is never automatically resubmitted without approved recovery evidence.
- The RandomX upstream adapter remains intentionally separate from Bitcoin Stratum V1. Runtime configuration can activate only the local laboratory and rejects `NODE_ENV=production` in code.
...
- Payout activation-readiness unit coverage passes 5/5, including a completely healthy disposable boundary, aggregate blocker reporting, future/stale reconciliation rejection, non-zero wallet variance, inconsistent wallet reconciliation timestamps, and an unresolved post-signer quarantine.
- Current-tree public browser smoke coverage passes 12/12 across desktop Chrome and Pixel 7 profiles, including route readiness, mobile overflow, protected-route redirect, real authentication fields, and fail-closed omission of Google Sign-In while OAuth is disabled. Web service coverage passes 4/4 and proves one payout request identity survives ambiguous exact-intent retries while account or amount changes rotate it. The authenticated no-mock journey remains independently covered on both profiles and must be rerun by exact-commit CI after schema 23 is committed.
- Exact-tree schema-v23 fresh and representative alpha.7/schema-v13 staged upgrade rehearsals apply all 23 migrations, preserve historical payout, native-submission, accepted-share, submission-intent, and financial evidence fixtures, verify dispatch-fingerprint, contribution-event, payout-recovery, and OAuth identity foundations, and exercise all RandomX submission/outbox/contribution plus payout-control triggers. A negative rehearsal also proves a non-`public` `DATABASE_URL` is rejected before the first database command, matching the explicit historical-migration compatibility boundary.
...
- The PostgreSQL-plus-Redis RandomX gateway suite passes 44/44, including a real TCP miner → production credential → private job → loopback sidecar validation → dedicated upstream submission → durable intent/decision/outbox runtime trace. Coverage also proves authenticated identity/account/pool binding, authoritative target/job/session evidence, disconnect races, connection/line/share bounds, cross-worker duplicate suppression, durable retry after job eviction, authorization and session races, local/upstream rejection, transport ambiguity, transaction rollback, immutable envelopes, Redis-time cross-client collision exclusion, ownership loss, renewal, compare-and-delete release, complete dependency shutdown, and absence of reward, journal, reservation, or balance effects before settlement.
- Wallet-worker boundary/encryption/signer tests pass 9/9 and Bitcoin adapter coverage passes 24/24, including authenticated artifact-context isolation, current destination/route/wallet/signer authorization, malicious signer-output rejection, watch-wallet change ownership, finalized transaction-intent equality, emergency action gates, bounded streamed responses, exact UTXO-set accounting, and raw-transaction txid recovery. A clean isolated PostgreSQL plus Bitcoin Core run completes the entire payout, forced-reorg, reconfirmation, legacy raw-only ambiguous-broadcast observation recovery, row-lock exclusion of a concurrent destination revocation during signing, and post-signer quarantine while preserving the active reservation and preventing broadcast; the dedicated GitHub payout-regtest job reproduces that evidence independently from the local Docker Desktop state.
- Frozen installation passes across all 35 workspaces; repository lint and production build pass 34/34, while the full typecheck and test dependency graphs pass 53/53 with 21 generated Next.js routes. Google OAuth configuration/integration, wallet-network validation, payout-control, RandomX, and build-info version tests all pass with the alpha.8/schema-23 identity.
- Managed-source and release manifests verify at 719 and 720 files respectively; the dedicated alpha.8 static checker passes and requires version `0.3.0-alpha.8`, schema 23, the OAuth migration, the alpha.7 upgrade boundary, and the Codex assistance disclosure.
- Docker E2E is configured to apply all 23 migrations to a clean PostgreSQL volume, build the API, web, outbox, mining, accounting, and isolated wallet/signing boundaries from the frozen lockfile, start the production reverse-proxy stack, reach API readiness, and verify public version plus fail-closed wallet-status responses before deleting the disposable containers and volumes. Exact-commit Docker and Vercel CI evidence remains a promotion gate.
