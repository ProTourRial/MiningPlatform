...
- RandomX validation is not yet connected to a miner-facing listener or production sidecar. A canonical event factory and strict evidence-only consumer now exist, but no runtime producer, durable pre-RPC submission intent, or transactional outbox emits that event; the path cannot create a contribution, journal, reward, payout, or balance in this checkpoint.
...
- RandomX unit coverage passes 11/11, including canonical event construction and rejection of missing/mutated work; integration proves eight concurrent repository writes and four concurrent event deliveries each produce one evidence row, altered payload reuse conflicts, untrusted/unknown payloads fail closed, persistence failure rolls back event idempotency, and mining-worker passes 9/9.
...
- Static alpha.7 checks, the 602-file managed-source manifest, and the 603-file release manifest verify against the final checkpoint tree.
