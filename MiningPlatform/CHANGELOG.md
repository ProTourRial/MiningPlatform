...
- RandomX validation is not yet connected to a miner-facing listener or production sidecar. A dormant runtime producer boundary now persists pre-RPC intent and accepted decisions through a transactional outbox, but it is not wired into production services and cannot create a contribution, journal, reward, payout, or balance in this checkpoint. Any failure after durable intent—including a pre-write session/job change whose non-dispatch is known locally—remains unresolved by policy and is never automatically resubmitted until an operator-owned recovery state machine exists.
...
- Exact-tree schema-v20 fresh and representative schema-v18/v19 upgrade rehearsals apply all 20 migrations, preserve historical payout, native-submission, accepted-share, and submission-intent fixtures, verify the dispatch-fingerprint backfill, and exercise all RandomX submission/outbox triggers.
