...
- Wallet-worker boundary/encryption/signer tests pass 8/8 and Bitcoin adapter coverage passes 22/22, including authenticated artifact-context isolation, emergency action gates, bounded streamed responses, exact UTXO-set accounting, and raw-transaction txid recovery. A clean local PostgreSQL plus Bitcoin Core run completes the entire payout, forced-reorg, and reconfirmation trace without warnings; the dedicated GitHub payout-regtest job reproduces that evidence independently from the local Docker Desktop state.
