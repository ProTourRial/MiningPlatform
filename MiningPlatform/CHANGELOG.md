...
- Active and packaged RandomX CI gates that start disposable PostgreSQL, apply the complete migration chain, build the exact dependency graph, and exercise validator, upstream protocol, bounded miner transport, durable submission/outbox, and contribution hand-off without starting a public listener.
...
- RandomX unit coverage passes 16/16, CryptoNote upstream adapter coverage passes 19/19, and the RandomX gateway suite passes 18/18: eight bounded miner-transport tests plus ten durable submission integrations covering authenticated identity/account/pool binding, authoritative target/job/session evidence, disconnect races, connection/line/share bounds, cross-worker duplicate suppression, durable retry after job eviction, authorization and session races, local/upstream rejection, transport ambiguity, transaction rollback, immutable envelopes, and explicit absence of reward, journal, reservation, or balance effects before settlement.
