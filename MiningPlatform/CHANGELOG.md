...
- The new RandomX miner-facing transport is not yet runtime-activated or connected to the production credential limiter, authoritative distinct-work source, or production sidecar. A Redis-backed cross-replica collision guard now exists, but it deliberately rejects duplicate nonce-normalized blobs instead of inventing an upstream-incompatible nonce range or modifying work locally. The dormant producer can create an immutable contribution only after local and upstream acceptance; rewards, journals, balances, and payouts still require an independently imported, exactly matched upstream settlement. Any failure after durable intent—including a pre-write session/job change whose non-dispatch is known locally—remains unresolved by policy and is never automatically resubmitted until an operator-owned recovery state machine exists.
...
- RandomX unit coverage passes 16/16, CryptoNote upstream adapter coverage passes 19/19, and the PostgreSQL-plus-Redis RandomX gateway suite passes 25/25: nine bounded miner-transport tests, ten durable submission integrations, and six work-isolation tests covering authenticated identity/account/pool binding, authoritative target/job/session evidence, disconnect races, connection/line/share bounds, cross-worker duplicate suppression, durable retry after job eviction, authorization and session races, local/upstream rejection, transport ambiguity, transaction rollback, immutable envelopes, Redis-time cross-client collision exclusion, ownership loss, renewal, compare-and-delete release, complete dependency shutdown, and explicit absence of reward, journal, reservation, or balance effects before settlement.
