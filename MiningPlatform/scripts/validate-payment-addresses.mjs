...
function validateBase58Check(address, expectedVersion) {
  ...
  if (!doubleSha256(payload).subarray(0, 4).equals(checksum))
    throw new Error(`${address} has an invalid checksum`);
  if (payload[0] !== expectedVersion)
    throw new Error(`${address} has unexpected network version ${payload[0]}`);
}
...
if (addresses.length !== 9)
  throw new Error(`Expected 9 configured address entries, found ${addresses.length}`);
...
if (decodeBase58(solana).length !== 32 || !addresses.includes(solana))
  throw new Error('Solana receiving address is invalid');
...
if (addresses.filter((value) => value === bitcoin).length !== 2) {
  throw new Error('Bitcoin receiving and native coinbase destinations are not aligned');
}
