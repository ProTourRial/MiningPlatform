...
16. The miner-facing transport is a separate bounded CryptoNote JSON-RPC server. It accepts login,
    ...
    below are satisfied.
17. A distributed Redis lease guards the miner work search space before an assignment is exposed. The
    lease fingerprint binds the algorithm, seed hash, and hashing blob after the four-byte CryptoNote
    nonce is zeroed; job identifiers, height, and target are excluded because changing routing metadata
    or share difficulty does not create different hash input. Redis server time bounds each lease to five
    minutes, acquisition and renewal are atomic, submission resolution rechecks ownership, and release is
    compare-and-delete. An identical active fingerprint on another connection or replica fails closed.
    The guard never mutates an upstream blob and cannot make shared work unique by assertion: the injected
    provider must supply distinct upstream-authoritative blobs, such as work generated by a verified
    provider-side extra-nonce mechanism.
...
- Accounting evidence now has deterministic projection, canonical event construction, append-only
  ...
  bounded but inactive miner-facing transport. The Redis lease prevents concurrent reuse of identical
  search space but does not synthesize unique upstream work. Production credential wiring, a verified
  distinct-work provider, runtime activation, unresolved-intent operator recovery, reward-period
  assignment, settlement reconciliation, and ledger effects remain intentionally gated.
...
- Wire a provider that supplies distinct upstream-authoritative hashing blobs. Retain the Redis collision
  guard and prove its ownership, TTL, reconnect, process-restart, Redis failover, and partition behavior;
  standard CryptoNote Stratum does not advertise a safe local nonce-range partition to invent here.
