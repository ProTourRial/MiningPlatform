...
18. The first concrete work source opens a separate CryptoNote upstream authorization session for each
    authenticated miner connection. It snapshots every provider-issued job behind a private miner job
    identifier, bounds retained mappings, verifies that a mapping is still present in the exact owning
    adapter session before submission, and builds one durable submission coordinator per adapter. A
    disconnect clears all mappings and the next assignment establishes a fresh session. Provider job
...
- Configure the dedicated-session source against a provider that demonstrably supplies distinct
  ...
  not advertise a safe local nonce-range partition to invent here.
