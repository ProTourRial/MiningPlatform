...
function reject(code, safeReason, fingerprint) {
    return { accepted: false, state: 'LOCAL_REJECTED', code, safeReason, fingerprint };
}
export class BitcoinShareValidationService {
    duplicates;
    now;
    maximumFutureTimeSeconds;
    constructor(duplicates, now = () => new Date(), maximumFutureTimeSeconds = 7_200) {
        this.duplicates = duplicates;
        this.now = now;
        this.maximumFutureTimeSeconds = maximumFutureTimeSeconds;
    }
    async validate(context) {
        let state = transitionShareState('RECEIVED', 'VALIDATING');
        const rejectCurrent = (code, reason, fingerprint) => {
        ...
        if (!job || submission.jobId !== job.id)
            return rejectCurrent('UNKNOWN_JOB', 'Mining job is unknown');
        ...
        if (!reserved)
            return rejectCurrent('DUPLICATE', 'Share was already submitted', fingerprint);
        try {
    ...
    async releaseReservation(fingerprint) {
        await this.duplicates.release(fingerprint);
    }
}
//# sourceMappingURL=share-validation-service.js.map