...
export function parseStratumMessage(line) {
    ...
    if (typeof value.method === 'string') {
        if (!Array.isArray(value.params))
            throw new Error('Invalid Stratum request parameters');
        return { id: value.id ?? null, method: value.method, params: value.params };
    }
    if (!('result' in value) || !('error' in value))
        throw new Error('Invalid Stratum response');
...
export function parseStratumLine(line) {
    const message = parseStratumMessage(line);
    if (!('method' in message))
        throw new Error('Expected a Stratum request or notification');
    return message;
}
export function parseMiningConfigure(params) {
    ...
    return { extensions: [...extensions], options: options };
}
export function parseMiningSubscribe(params) {
    const userAgent = params[0];
    const sessionId = params[1];
    if (userAgent !== undefined && typeof userAgent !== 'string')
        throw new Error('Invalid miner user agent');
    if (sessionId !== undefined && typeof sessionId !== 'string')
        throw new Error('Invalid subscription session id');
    return { userAgent, sessionId };
}
export function parseMiningAuthorize(params) {
...
export function parseMiningSubmit(params) {
...
export function parseMiningSubscribeResult(result) {
    if (!Array.isArray(result) || result.length < 3)
        throw new Error('Invalid mining.subscribe result');
    const [rawSubscriptions, rawExtranonce1, rawSize] = result;
    if (!Array.isArray(rawSubscriptions))
        throw new Error('Invalid subscription list');
    const subscriptions = rawSubscriptions.map((entry) => {
...
export function parseMiningSetDifficulty(params) {
...
export function parseMiningSetExtranonce(params) {
...
export function parseMiningNotify(params) {
    if (params.length < 9)
        throw new Error('Invalid mining.notify parameter count');
    const [jobId, prevhash, coinbase1, coinbase2, branches, version, bits, time, cleanJobs] = params;
    if (typeof jobId !== 'string' || jobId.length === 0)
        throw new Error('Invalid mining.notify job id');
    if (!Array.isArray(branches))
        throw new Error('Invalid mining.notify merkle branch list');
    if (typeof cleanJobs !== 'boolean')
        throw new Error('Invalid mining.notify clean_jobs flag');
    return {
        jobId,
...
export function serializeStratumRequest(request) {
    return `${JSON.stringify(request)}\n`;
}
export function serializeStratumResponse(response) {
    return `${JSON.stringify(response)}\n`;
}
export function serializeStratumNotification(method, params) {
    const notification = { id: null, method, params };
