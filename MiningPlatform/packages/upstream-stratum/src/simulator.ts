...
export class UpstreamStratumSimulator {
  ...
  constructor(private readonly options: UpstreamSimulatorOptions = {}) {
    this.currentJob = createDevelopmentJob(
      new Date(),
      options.difficulty ?? '0.000001',
      options.extranonce1 ?? 'e9695791',
    );
    ...
    await new Promise<void>((resolve, reject) => {
      ...
      this.server.listen(this.options.port ?? 0, this.options.host ?? '127.0.0.1', () => {
        ...
        if (!address || typeof address === 'string')
          return reject(new Error('Simulator did not expose a TCP port'));
  ...
  rotateJob(cleanJobs = true): BitcoinMiningJob {
    const now = new Date();
    const next = createDevelopmentJob(
      now,
      this.options.difficulty ?? '0.000001',
        ...
        if (line) {
          ...
          switch (request.method) {
            ...
            case 'mining.authorize': {
              ...
              authorized =
                credentials.workerName === (this.options.username ?? 'upstream.account') &&
                credentials.password === (this.options.password ?? 'x');
              socket.write(serializeStratumResponse(successResponse(request.id, authorized)));
              if (authorized) {
                socket.write(
                  serializeStratumNotification('mining.set_difficulty', [
                    this.options.difficulty ?? '0.000001',
                  ]),
                );
                socket.write(
                  serializeStratumNotification('mining.set_extranonce', [
                    this.options.extranonce1 ?? 'e9695791',
                    this.options.extranonce2Size ?? 4,
                  ]),
                );
            ...
            case 'mining.submit': {
              if (!authorized) {
                socket.write(
                  serializeStratumResponse(errorResponse(request.id, 24, 'Unauthorized worker')),
                );
                break;
              }
              ...
              if (this.options.rejectShares || submission.jobId !== this.currentJob.id) {
                socket.write(
                  serializeStratumResponse(
                    errorResponse(request.id, 21, 'Stale or rejected share'),
                  ),
                );
              } else {
            ...
            default:
              socket.write(
                serializeStratumResponse(errorResponse(request.id, 20, 'Unsupported method')),
              );
          }
        }
  ...
  private notifyJob(socket: Socket, job: BitcoinMiningJob): void {
    socket.write(
      serializeStratumNotification('mining.notify', [
        job.id,
