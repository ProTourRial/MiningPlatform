...
function findShare(job: BitcoinMiningJob): BitcoinShareSubmission {
  for (let nonce = 0; nonce < 2_000_000; nonce += 1) {
    ...
    if (
      calculateHeaderHash(job, submission).numericValue
      <= DIFFICULTY_ONE_TARGET * 1_000_000n
    ) {
      return submission;
    }
  }
...
test(
  'gateway job router produces provider-scoped ids and invalidates old clean jobs',
  () => {
    ...
    assert.notEqual(
      primary.downstreamJob.id,
      backup.downstreamJob.id,
    );
    ...
    assert.equal(
      router.resolve(cleanReplacement.downstreamJob.id)?.poolId,
      'backup-pool',
    );
  },
);
...
test(
  'VarDiff never drops below the active upstream difficulty floor',
  () => {
...
test(
  'multi-upstream manager keeps the session alive and fails over to backup',
  async () => {
    ...
    const manager = new MultiUpstreamPoolManager(
      {
        pools: [
          pool('primary', primary.port, 10),
          pool('backup', backup.port, 20),
        ],
    ...
    try {
      ...
      await waitUntil(
        () =>
          manager.activePool === 'backup'
          && manager.currentState === 'ACTIVE',
      );

      await waitUntil(
        () => jobs.some((job) => job.extranonce1 === '22222222'),
      );

      assert.deepEqual(
        activePools.slice(0, 2),
        ['primary', 'backup'],
      );

      const backupJob = [...jobs]
        .reverse()
        .find((job) => job.extranonce1 === '22222222');
      ...
      assert.equal(
        manager.health()
          .find((entry) => entry.poolId === 'primary')
          ?.state,
        'CIRCUIT_OPEN',
      );
    } finally {
