...
const platformFeatures: Array<{ icon: LucideIcon; title: string; description: string; tag: string }> = [
  {
    icon: Network,
    title: 'Stratum Gateway',
    description: 'Menerima koneksi worker, mengelola sesi, memvalidasi share lokal, lalu meneruskan pekerjaan ke upstream pool.',
    tag: 'Mining core',
  },
  {
    icon: Cpu,
    title: 'Farm Monitoring',
    description: 'Menggabungkan data Stratum dan agent perangkat untuk menampilkan hashrate, uptime, suhu, fan, daya, dan efisiensi.',
    tag: 'Realtime',
  },
  {
    icon: DatabaseZap,
    title: 'Auditable Accounting',
    description: 'Reward, fee, dan kewajiban payout dicatat melalui jurnal double-entry. Wallet tidak mengubah saldo user secara langsung.',
    tag: 'Ledger',
  },
  {
    icon: ShieldCheck,
    title: 'Operational Control',
    description: 'Owner mengelola maintenance, upstream health, emergency stop, audit log, dan tindakan sensitif melalui akses privat.',
    tag: 'Security',
  },
];
...
const pipelineSteps = [
  ...
  ['05', 'Aggregate', 'Difficulty terakumulasi menjadi bucket hashrate dan statistik worker untuk beberapa window waktu.'],
  ['06', 'Settle Later', 'Reward baru dihitung setelah data upstream direkonsiliasi. Ledger dan wallet berada pada fase sesudahnya.'],
] as const;
...
export function LandingSections() {
  ...
  return (
    <>
        ...
        <div className="relative mx-auto max-w-[1380px] px-5 sm:px-8">
          ...
          <div className="mt-12 grid gap-4 md:grid-cols-2 xl:grid-cols-4">
            {platformFeatures.map(({ icon: Icon, title, description, tag }) => (
              <article key={title} className="group rounded-2xl border border-white/10 bg-[#091827]/72 p-6 transition hover:-translate-y-1 hover:border-[#d7ff63]/35 hover:bg-[#0b1b2c]">
                <div className="flex items-center justify-between">
                  ...
                  <span className="mono-font text-[9px] font-semibold uppercase tracking-[0.16em] text-[#72899d]">{tag}</span>
                </div>
                <h3 className="display-font mt-8 text-2xl font-extrabold tracking-[-0.035em] text-white">{title}</h3>
      ...
      <section id="pipeline" className="relative overflow-hidden border-b border-white/10 bg-[#081522] py-24 sm:py-30">
          ...
          <div className="grid gap-12 lg:grid-cols-[0.76fr_1.24fr]">
            <div className="lg:sticky lg:top-28 lg:self-start">
              ...
              <div className="mt-8 rounded-2xl border border-[#d7ff63]/18 bg-[#d7ff63]/6 p-5">
                <p className="mono-font text-[10px] font-bold uppercase tracking-[0.16em] text-[#d7ff63]">Current release boundary</p>
                <p className="mt-3 text-sm leading-6 text-[#b9c9d5]">
                  v0.2 berfokus pada Stratum, share validation, durable intake, upstream response, hashrate, WebSocket, dan dashboard realtime.
                </p>
              </div>
            </div>
            ...
            <div className="space-y-3">
              {pipelineSteps.map(([number, title, description], index) => (
                <article key={number} className="group grid gap-4 rounded-2xl border border-white/9 bg-[#0a1929]/76 p-5 transition hover:border-[#98f5ff]/28 sm:grid-cols-[76px_1fr_auto] sm:items-center sm:p-6">
                  <div className="mono-font text-3xl font-black tracking-[-0.06em] text-[#263e52] transition group-hover:text-[#d7ff63]">{number}</div>
                  <div>
                    <h3 className="display-font text-xl font-extrabold tracking-[-0.025em] text-white">{title}</h3>
                    <p className="mt-2 text-sm leading-6 text-[#a9bbc9]">{description}</p>
                  </div>
                  <span className={`mono-font w-fit rounded-full border px-3 py-1.5 text-[9px] font-bold uppercase tracking-[0.14em] ${index < 2 ? 'border-[#d7ff63]/25 bg-[#d7ff63]/8 text-[#d7ff63]' : index < 5 ? 'border-[#98f5ff]/22 bg-[#98f5ff]/7 text-[#98f5ff]' : 'border-white/10 bg-white/4 text-[#71899d]'}`}>
            ...
            <div>
              ...
              <div className="mt-9 grid grid-cols-2 gap-3">
                {monitoringMetrics.map(({ icon: Icon, label, value, note }) => (
                  <div key={label} className="rounded-2xl border border-white/9 bg-[#091827]/72 p-4 sm:p-5">
                    <div className="flex items-center justify-between text-[#98f5ff]">
                      <Icon size={17} />
                      <span className="mono-font text-[9px] uppercase tracking-[0.12em] text-[#70879a]">{note}</span>
                    </div>
            ...
            <div className="overflow-hidden rounded-[26px] border border-white/12 bg-[#081522] shadow-2xl shadow-black/25">
              <div className="flex items-center justify-between border-b border-white/9 px-5 py-4">
                <div>
                  ...
                  <p className="mono-font mt-1 text-[9px] uppercase tracking-[0.14em] text-[#71899c]">Realtime dashboard preview</p>
                </div>
                <span className="inline-flex items-center gap-2 text-xs font-semibold text-[#8da3b4]"><span className="size-2 rounded-full bg-[#526b7d]" /> No worker connected</span>
              </div>
              ...
              <div className="grid gap-px bg-white/8 sm:grid-cols-3">
                {[
                  ...
                  <div key={label} className="bg-[#0a1928] px-5 py-5">
                    <p className="mono-font text-[9px] uppercase tracking-[0.14em] text-[#70879a]">{label}</p>
              ...
              <div className="p-5 sm:p-6">
                <div className="relative h-52 overflow-hidden rounded-2xl border border-white/8 bg-[#06111f] p-4">
                  ...
                  <svg viewBox="0 0 600 180" className="relative h-full w-full" aria-label="Development hashrate chart placeholder">
                    <defs>
                    ...
                    <path d="M0 146 C70 145 86 128 142 132 S230 112 286 119 S371 84 430 95 S520 54 600 66" fill="none" stroke="url(#chartLine)" strokeWidth="4" strokeLinecap="round" opacity="0.86" />
                    <path d="M0 146 C70 145 86 128 142 132 S230 112 286 119 S371 84 430 95 S520 54 600 66 L600 180 L0 180 Z" fill="url(#chartLine)" opacity="0.06" />
                  </svg>
                ...
                <div className="mt-4 grid grid-cols-2 gap-3 sm:grid-cols-4">
                  {['Online', 'Accepted', 'Rejected', 'Last share'].map((label) => (
                    <div key={label} className="rounded-xl border border-white/8 bg-white/[0.025] p-3">
                      <p className="mono-font text-[8px] uppercase tracking-[0.12em] text-[#6d8598]">{label}</p>
                      <p className="mt-2 text-sm font-bold text-[#e9f3f8]">{label === 'Last share' ? 'Never' : '0'}</p>
                    </div>
                  ))}
                </div>
              </div>
      ...
      <section id="simulator" className="relative overflow-hidden border-b border-white/10 bg-[#081522] py-24 sm:py-30">
          ...
          <div className="grid gap-12 lg:grid-cols-[0.9fr_1.1fr]">
            <div>
              ...
              <div className="mt-8 flex flex-wrap gap-2">
                {['Hashrate', 'Power', 'Electricity', 'Difficulty', 'Block reward', 'Pool fee'].map((item) => (
                  <span key={item} className="mono-font rounded-full border border-white/10 bg-white/[0.035] px-3 py-2 text-[9px] font-semibold uppercase tracking-[0.13em] text-[#91a6b6]">{item}</span>
                ))}
              </div>
            </div>
            ...
            <div className="rounded-[26px] border border-white/12 bg-[#0a1928]/82 p-5 sm:p-7">
              <div className="flex items-center justify-between border-b border-white/9 pb-5">
                <div className="flex items-center gap-3">
                  <span className="grid size-10 place-items-center rounded-xl bg-[#d7ff63]/10 text-[#d7ff63]"><Calculator size={19} /></span>
                  <div>
                    <p className="font-bold text-white">Profitability Preview</p>
                    <p className="mono-font mt-1 text-[9px] uppercase tracking-[0.14em] text-[#71899c]">Analytical output only</p>
                  </div>
                </div>
                <span className="mono-font text-[9px] uppercase tracking-[0.14em] text-[#d7ff63]">BTC / SHA-256</span>
              </div>
              ...
              <div className="mt-4 rounded-2xl border border-[#98f5ff]/17 bg-[#98f5ff]/6 p-5">
                <div className="flex items-end justify-between gap-4">
                  <div>
                    <p className="mono-font text-[9px] uppercase tracking-[0.13em] text-[#7ea3ad]">Estimated net result</p>
                    <p className="mt-2 text-3xl font-black tracking-[-0.04em] text-white">Belum dihitung</p>
                  </div>
                  <BarChart3 className="text-[#98f5ff]" />
                </div>
                <p className="mt-4 text-xs leading-5 text-[#91a9b7]">Nilai aktual membutuhkan sumber harga, difficulty, block reward, dan tarif listrik yang terverifikasi.</p>
              </div>
            </div>
          </div>
        </div>
      ...
      <section id="transparency" className="relative border-b border-white/10 py-24 sm:py-30">
        <div className="mx-auto max-w-[1380px] px-5 sm:px-8">
          <div className="flex flex-col gap-8 lg:flex-row lg:items-end lg:justify-between">
            ...
            />
            <Link href="/transparency" className="group inline-flex shrink-0 items-center gap-2 text-sm font-bold text-[#d7ff63]">
              Buka dashboard transparansi <ArrowUpRight size={17} className="transition group-hover:translate-x-0.5 group-hover:-translate-y-0.5" />
            </Link>
          </div>
          ...
          <div className="mt-12 grid grid-cols-2 gap-px overflow-hidden rounded-2xl border border-white/10 bg-white/8 md:grid-cols-3 lg:grid-cols-6">
            {transparencyMetrics.map(([label, value]) => (
              <div key={label} className="bg-[#091827] p-5 sm:p-6">
                <p className="mono-font text-[9px] uppercase tracking-[0.13em] text-[#70879a]">{label}</p>
          ...
          <div className="mt-4 flex items-start gap-3 rounded-xl border border-amber-300/15 bg-amber-300/5 px-4 py-3.5 text-xs leading-5 text-amber-100/78">
            ...
            Nilai di atas merupakan placeholder development. Data produksi baru dapat ditampilkan setelah upstream connector, durable intake, dan otorisasi monitoring selesai.
          </div>
        </div>
      </section>
      ...
      <section className="border-b border-white/10 bg-[#081522] py-24 sm:py-30">
        <div className="mx-auto max-w-[1380px] px-5 sm:px-8">
          ...
          <div className="mt-12 grid gap-4 md:grid-cols-2 lg:grid-cols-4">
            {[
              { icon: Layers3, title: 'Domain first', text: 'Repository menangani persistence. Domain service menangani aturan bisnis dan state transition.' },
              { icon: HardDrive, title: 'Durable by design', text: 'Share accepted, outbox, idempotency, dan recovery dirancang untuk menghadapi retry serta process crash.' },
              { icon: WalletCards, title: 'Wallet isolation', text: 'Wallet hanya menandatangani dan broadcast transaksi. Saldo user hanya berubah melalui ledger journal.' },
              { icon: LockKeyhole, title: 'Private operations', text: 'Owner access memakai autentikasi kuat, audit log, step-up verification, dan jaringan terbatas.' },
            ].map(({ icon: Icon, title, text }) => (
              <article key={title} className="rounded-2xl border border-white/9 bg-[#0a1928]/70 p-6">
                <Icon size={21} className="text-[#98f5ff]" />
                <h3 className="display-font mt-7 text-xl font-extrabold tracking-[-0.025em] text-white">{title}</h3>
        ...
        <div className="relative mx-auto grid max-w-[1380px] gap-12 px-5 sm:px-8 lg:grid-cols-[0.75fr_1.25fr]">
          ...
          <div className="space-y-3">
            {[
              ...
              <details key={question} className="group rounded-2xl border border-white/9 bg-[#091827]/72 p-5 open:border-[#98f5ff]/22 open:bg-[#0b1b2b] sm:p-6">
                <summary className="flex cursor-pointer list-none items-center justify-between gap-5 font-bold text-[#edf6fa]">
                  {question}
                  <span className="mono-font text-lg text-[#d7ff63] transition group-open:rotate-45">+</span>
                </summary>
      ...
      <section className="px-5 pb-20 sm:px-8 sm:pb-28">
        <div className="relative mx-auto max-w-[1380px] overflow-hidden rounded-[28px] border border-[#d7ff63]/22 bg-[#0a1928] px-6 py-12 sm:px-10 lg:px-14 lg:py-16">
          ...
          <div className="relative flex flex-col gap-8 lg:flex-row lg:items-end lg:justify-between">
            <div>
              <p className="mono-font text-[10px] font-bold uppercase tracking-[0.18em] text-[#d7ff63]">Development access</p>
...
function SectionHeading({ eyebrow, title, description }: { eyebrow: string; title: string; description: string }) {
  return (
    <div className="max-w-3xl">
      <p className="mono-font text-[10px] font-bold uppercase tracking-[0.2em] text-[#d7ff63]">{eyebrow}</p>
      <h2 className="display-font mt-4 text-[clamp(2.45rem,5vw,5.25rem)] font-black uppercase leading-[0.94] tracking-[-0.055em] text-[#f5fbff]">{title}</h2>
