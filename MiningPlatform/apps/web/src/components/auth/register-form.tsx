    ...
    <form className="space-y-4" onSubmit={submit}>
      <label className="block text-sm">
        ...
        <input
          ...
          required
          minLength={2}
          className="mt-2 w-full rounded-xl border border-white/10 bg-black/20 px-4 py-3 outline-none focus:border-[var(--accent)]"
        />
      </label>
      <label className="block text-sm">
        Email
      ...
      <label className="block text-sm">
        ...
        <span className="mt-1 block text-xs text-[var(--muted)]">
          3–32 karakter: huruf kecil, angka, garis bawah, atau tanda hubung.
        </span>
      </label>
      <label className="block text-sm">
        Password
      ...
      <button
        disabled={submitting}
        className="w-full rounded-xl bg-[var(--accent)] px-4 py-3 font-semibold text-[#04110c] disabled:opacity-60"
      >
        {submitting ? 'Membuat akun…' : 'Buat akun'}
      </button>
      {message && (
        <p className="rounded-xl border border-white/10 bg-white/5 p-3 text-xs leading-6">
          {message}
        </p>
      )}
      {token && (
