...
class FakeStore implements WorkerCredentialStore {
  ...
  async recordSuccess(candidate: WorkerCredentialCandidate): Promise<{ accepted: true }> {
...
async function candidate(
  overrides: Partial<WorkerCredentialCandidate> = {},
): Promise<WorkerCredentialCandidate> {
  return {
    workerId: 'worker-1',
...
test('production authenticator accepts an active worker credential and clears limiter state', async () => {
  ...
  const result = await authenticator.authenticate(
    'account.worker1',
    'mpw_valid-test-secret-123456789012345',
    context,
  );
...
test('worker identity parser separates and normalizes a referral suffix', () => {
  assert.deepEqual(parseWorkerIdentity('account.worker1#mp05'), {
    accountUsername: 'account',
    workerName: 'worker1',
    referralCode: 'MP05',
  });
  assert.equal(parseWorkerIdentity('account.worker1#bad-code'), null);
  assert.equal(parseWorkerIdentity('account.worker1#MP05#SECOND'), null);
});

test('production authenticator preserves the normalized referral suffix for share authorization', async () => {
  const store = new FakeStore([await candidate()]);
  const limiter = new InMemoryWorkerAuthRateLimiter(2, 60_000, 900_000);
  const authenticator = new ProductionWorkerAuthenticator(config(), store, limiter);
  const result = await authenticator.authenticate(
    'account.worker1#mp05',
    'mpw_valid-test-secret-123456789012345',
    context,
  );
  assert.equal(result.authenticated, true);
  if (result.authenticated) assert.equal(result.worker.workerName, 'account.worker1#MP05');
});
...
test('production authenticator records invalid credentials without revealing worker existence', async () => {
  ...
  const result = await authenticator.authenticate(
    'account.worker1',
    'wrong-secret-with-enough-characters',
    context,
  );
...
test('production authenticator rate-limits repeated failures', async () => {
  ...
  await authenticator.authenticate(
    'account.worker1',
    'wrong-secret-one-with-enough-characters',
    context,
  );
  await authenticator.authenticate(
    'account.worker1',
    'wrong-secret-two-with-enough-characters',
    context,
  );
  const blocked = await authenticator.authenticate(
    'account.worker1',
    'mpw_valid-test-secret-123456789012345',
    context,
  );
